...
double* splinter_bspline_get_knot_vectors(splinter_obj_ptr bspline_ptr) {
    ...
    if (bspline != nullptr) {
        try {
            ...
            knot_vectors_as_array =
                (double*)malloc(total_n_elements * sizeof(double));
    ...
    if (bspline != nullptr) {
        try {
            ...
            coefficients_as_array =
                (double*)malloc(coefficients.size() * sizeof(double));
...
double* splinter_bspline_get_control_points(splinter_obj_ptr bspline_ptr) {
    ...
    if (bspline != nullptr) {
        try {
            ...
            control_points_as_array =
                (double*)malloc(control_points.size() * sizeof(double));

            if (control_points_as_array != nullptr) {
                for (int i = 0; i < control_points.rows(); ++i) {
                    for (int j = 0; j < control_points.cols(); ++j) {
                        control_points_as_array[i * control_points.cols() + j] =
                            control_points(i, j);
                    }
                }
            } else {
...
int* splinter_bspline_get_basis_degrees(splinter_obj_ptr bspline_ptr) {
    ...
    if (bspline != nullptr) {
        try {
            ...
            basis_degrees_as_array =
                (int*)malloc(basis_degrees.size() * sizeof(int));
...
double* splinter_bspline_eval_row_major(
...
double* splinter_bspline_eval_jacobian_row_major(
    splinter_obj_ptr bspline_ptr, double* x, int x_len) {
    double* retVal = nullptr;
    ...
    if (bspline != nullptr) {
        try {
            ...
            retVal =
                (double*)malloc(sizeof(double) * num_variables * num_points);
            for (size_t i = 0; i < num_points; ++i) {
                auto xvec = get_densevector<double>(x, num_variables);
...
double* splinter_bspline_eval_hessian_row_major(
    splinter_obj_ptr bspline_ptr, double* x, int x_len) {
    double* retVal = nullptr;
    ...
    if (bspline != nullptr) {
        try {
            ...
            retVal = (double*)malloc(
                sizeof(double) * num_variables * num_variables * num_points);
            for (size_t i = 0; i < num_points; ++i) {
                auto xvec           = get_densevector<double>(x, num_variables);
                ...
                memcpy(
                    retVal + i * num_variables * num_variables,
                    hessian.data(),
                    sizeof(double) * num_variables * num_variables);
                x += num_variables;
            }
        } catch (const Exception& e) { set_error_string(e.what()); }
    }
...
double* splinter_bspline_eval_col_major(
    splinter_obj_ptr bspline_ptr, double* x, int x_len) {
    double* retVal = nullptr;
...
double* splinter_bspline_eval_jacobian_col_major(
    splinter_obj_ptr bspline_ptr, double* x, int x_len) {
    double* retVal = nullptr;
    ...
    if (bspline != nullptr) {
        ...
        try {
            ...
            retVal = splinter_bspline_eval_jacobian_row_major(
                bspline,
                row_major,
                x_len);
        } catch (const Exception& e) { set_error_string(e.what()); }
...
double* splinter_bspline_eval_hessian_col_major(
    splinter_obj_ptr bspline_ptr, double* x, int x_len) {
    double* retVal = nullptr;
    ...
    if (bspline != nullptr) {
        ...
        try {
            ...
            retVal = splinter_bspline_eval_hessian_row_major(
                bspline,
                row_major,
                x_len);
        } catch (const Exception& e) { set_error_string(e.what()); }
...
void splinter_bspline_insert_knots(
